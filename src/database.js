...
export const deleteUser = (userDbId) => {
  ...
  db.prepare('DELETE FROM registrations WHERE user_id = ?').run(userDbId);
  db.prepare('DELETE FROM withdrawals WHERE user_id = ?').run(userDbId);
  db.prepare('DELETE FROM users WHERE id = ?').run(userDbId);
};

export const getUsersPage = (page, pageSize = 20) => {
